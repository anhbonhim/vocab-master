# Requirements Archive: v1.0 VocabMaster Refactor & Audit
...
- [x] **ARCH-01**: Refactor large monolithic Compose screens (e.g., `HomeScreen.kt` and `QuizScreen.kt`) into Screen/Content patterns, separating state/events from pure UI layout.
- [x] **ARCH-02**: Eliminate all unsafe forced unwraps (`!!`) and raw unsafe casts (`as`) in ViewModels and UI, replacing them with safe type casting (`as?`) and Elvis operators.
- [x] **ARCH-03**: Expose UI state from ViewModels using structured, immutable `UiState` classes via Kotlin `StateFlow` (UDF pattern).
- [x] **ARCH-04**: Hoist scheduling and quiz logic from ViewModels into clean domain Use Cases (e.g. `SubmitReviewUseCase`).

### UX Flow & Redesign

- [x] **UX-01**: Implement smooth navigation transitions using type-safe argument passing APIs.
- [x] **UX-02**: Refactor Quiz flow to handle screen orientation changes without losing active session progress.
- [x] **UX-03**: Build user feedback states during quizzes (correct/incorrect answer highlights, card scheduling preview) with high-fidelity visual indicators.
- [x] **UI-01**: Standardize spacing, typography, and theme across all Compose screens using a clean, modern design system layout.

### Sync & Networking

- [x] **SYNC-01**: Verify data sync flow and ensure `SyncManager` handles backend request failures gracefully with retry mechanisms.
- [x] **SYNC-02**: Ensure bidirectional data synchronization does not corrupt or downgrade FSRS card states.
